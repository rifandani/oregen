...
import { HookTestEnum, LogLevelEnum } from "../../enums/global.enum";
    ...
    if (flags.describe) {
      try {
        ...
        if (!flags["dry-run"] && aiGeneratedTemplate) {
          if (["debug"].includes(flags["log-level"]))
            logger(
              `Output the OpenAI generated hook template file`,
              flags["log-level"]
            );
        ...
        logger(
          `OpenAI failed to create the "${filename}" hook with the provided description`,
          LogLevelEnum.error,
          error
        );
      }
